...
def split_idx(run, nruns, idx, seed=1337):
    ...
    train_idx = list(idx[:start]) + list(idx[end:])
    # print(train_idx)
    test_idx = list(idx[start:end])
    return train_idx, test_idx

def crossval_test(X, y, test_idx, seg_length=480, flatten=True, fix_offset=0):
    """
    ...
    if flatten:
        ...
        Xout = np.zeros((preshape, seg_length, X.shape[-2], X.shape[-1]))
        yout = np.zeros((preshape, y.shape[-1]))
    else:
        preshape = (len(test_idx), ntrials)
        Xout = np.zeros((preshape[0], preshape[1], seg_length, X.shape[-2], X.shape[-1]))
        yout = np.zeros((preshape[0], preshape[1], y.shape[-1]))
   # x    = np.zerps((seg_length,X.shape[-2],X.shape[-1]))
